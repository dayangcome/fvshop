<%@page language="java" contentType="text/html; character=UTF-8" pageEncoding="UTF-8" %>
<%@include file="/common/taglibs.jsp"%>
<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN"
        "http://www.w3.org/TR/html4/loose.dtd">
<html>
<head>
    <title>首页</title>
    <link type="text/css" rel="stylesheet" href="${ctx}/resource/user/css/style.css">
    <script src="${ctx}/resource/user/js/jquery-1.8.3.min.js"></script>
    <script src="${ctx}/resource/user/js/jquery.luara.0.0.1.min.js"></script>
    <link rel="shortcut icon" type="image/x-icon" href="${ctx}/resource/images/mylogo.ico" />
    <style>
        .n15Nav{
            width: 232px;
            display:block;
            line-height:45px;
            cursor:pointer;
            position: absolute;
            left: 230px;
            background-color: #0ab50a;
            display: none;
            border-left: #f1eeee solid 1px;
        }
        .n15Nav font{
            color: white;
        }
        .n15Nav:nth-child(4){
            top:45px;
        }
        .n15Nav:nth-child(5){
            top:90px;
        }
        .n15Nav:nth-child(6){
            top:135px;
        }
        .n15Nav:nth-child(7){
            top:180px;
        }
        .n15Nav img{float:right;margin-right:54px;}
        .n15Nav:hover{background:#fff;}
        .n15Nav:hover font{color:#333;}
        .n1Nav:hover .n15Nav{
            display: block;
        }
        .n15Nav:hover .n2Nav{
            display: block;
        }
    </style>
</head>
<body>
    <%@include file="/common/utop.jsp"%>
<!--导航条-->

    <div class="width100" style="height: 45px;background: #0ab50a;margin-top: 40px;position: relative;z-index: 100;">
    <!--中间的部分-->
    <div class="width1200 center_yh relative_yh" style="height: 45px;">
        <!--列表导航-->
        <div class="left_yh Selected" style="width: 230px;height: 45px;border-bottom: #ffffffb8 0.2px solid" id="hiddenShow">
            <!--头部的图标-->
            <img src="${ctx}/resource/user/images/cd.png" class="left_yh" style="margin-left: 24px;" alt="">
            <span class="block_yh left_yh fff" style="height: 45px;line-height: 44px;margin-left: 10px;">分类</span>
            <!--导航展开部分-->
            <div class="downSlide">
                <div class="n1Nav">
                    <font class="font1">水果</font>
                    <img src="${ctx}/resource/user/images/jt.png" alt="">
                    <c:forEach items="${lbs}" var="data" varStatus="l">
                        <div class="n15Nav">
                            <font>${data.father.name}</font>
                            <img src="${ctx}/resource/user/images/jt.png" alt="">
                            <div class="n2Nav">
                                <div class="n3Nav">
                                    <h3>${data.father.name}</h3>
                                    <c:forEach items="${data.childrens}" var="child" varStatus="ll">
                                        <a href="${ctx}/item/shoplist?categoryIdTwo=${child.id}">${child.name}</a>
                                    </c:forEach>
                                </div>
                            </div>
                        </div>
                    </c:forEach>
                    <div class="n15Nav">
                        <font>其他</font>
                        <img src="${ctx}/resource/user/images/jt.png" alt="">
                        <div class="n2Nav">
                            <div class="n3Nav">
                                <h3>其他</h3>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="n1Nav">
                    <font class="font1">蔬菜</font>
                    <img src="${ctx}/resource/user/images/jt.png" alt="">
                        <div class="n15Nav">
                            <font>叶菜类</font>
                            <img src="${ctx}/resource/user/images/jt.png" alt="">
                            <div class="n2Nav">
                                <div class="n3Nav">
                                    <h3>叶菜类</h3>
                                        <a href="${ctx}/item/shoplist?categoryIdTwo=${child.id}">菠菜</a>
                                        <a href="${ctx}/item/shoplist?categoryIdTwo=${child.id}">生菜</a>
                                </div>
                            </div>
                        </div>
                    <div class="n15Nav">
                        <font>根茎类</font>
                        <img src="${ctx}/resource/user/images/jt.png" alt="">
                        <div class="n2Nav">
                            <div class="n3Nav">
                                <h3>根茎类</h3>
                                <a href="${ctx}/item/shoplist?categoryIdTwo=${child.id}">胡萝卜</a>
                                <a href="${ctx}/item/shoplist?categoryIdTwo=${child.id}">白萝卜</a>
                                <a href="${ctx}/item/shoplist?categoryIdTwo=${child.id}">马铃薯</a>
                            </div>
                        </div>
                    </div>

                    <div class="n15Nav">
                        <font>茄果类</font>
                        <img src="${ctx}/resource/user/images/jt.png" alt="">
                        <div class="n2Nav">
                            <div class="n3Nav">
                                <h3>茄果类</h3>
                                <a href="${ctx}/item/shoplist?categoryIdTwo=${child.id}">西红柿</a>
                                <a href="${ctx}/item/shoplist?categoryIdTwo=${child.id}">青椒</a>
                                <a href="${ctx}/item/shoplist?categoryIdTwo=${child.id}">黄瓜</a>
                            </div>
                        </div>
                    </div>

                    <div class="n15Nav">
                        <font>叶菜类</font>
                        <img src="${ctx}/resource/user/images/jt.png" alt="">
                        <div class="n2Nav">
                            <div class="n3Nav">
                                <h3>叶菜类</h3>
                                <a href="${ctx}/item/shoplist?categoryIdTwo=${child.id}">菠菜</a>
                                <a href="${ctx}/item/shoplist?categoryIdTwo=${child.id}">生菜</a>
                            </div>
                        </div>
                    </div>

                </div>
                <div class="n1Nav">
                    <font class="font1">其他</font>
                    <img src="${ctx}/resource/user/images/jt.png" alt="">
                </div>
            </div>
        </div>
        <!--普通导航-->
        <div class="left_yh font16" id="pageNav">
            <a href="${ctx}/login/uIndex">首页</a>
            <a href="${ctx}/news/list">公告</a>
            <a href="${ctx}/message/add">留言</a>
            <a href="${ctx}/question/quest">答题</a>
            <a href="${ctx}/nutrition/nutrition">营养分析</a>
            <a href="${ctx}/community/community">社区论坛</a>
        </div>
    </div>
</div>
<!--轮动广告-->
<div class="width1200 center_yh hidden_yh" style="position: relative;z-index:80;">
    <div class="example2" style="width: 1200px;height: 490px;overflow: hidden;margin-left: 230px;">
        <ul>
            <li><img src="${ctx}/resource/images/a.webp" alt=""></li>
            <li><img src="${ctx}/resource/images/find1.jpg" alt=""></li>
            <li><img src="${ctx}/resource/images/find2.jpg" alt=""></li>
            <li><img src="${ctx}/resource/images/find3.jpg" alt=""></li>
        </ul>
    </div>
    <script>
        $(function () {
            $(".example2").luara({width:"966",height:"490",interval:4500,selected:"seleted",deriction:"left"});
        });
    </script>
</div>
<!--折扣商品-->
<div class="width1200 center_yh hidden_yh">
    <div class="width100" style="height: 45px;line-height: 45px;border-bottom: 2px solid #0ab50a; margin-top: 20px;">
        <font class="left_yh font20">折扣大促销</font>
    </div>
    <div class="width100 hidden_yh" style="height: 480px;">
        <div class="normalPic">
            <c:forEach items="${zks}" var="data" varStatus="l">
                <a href="${ctx}/item/view?id=${data.id}">
                    <h3 class="yihang c_33 font14 font100" style="padding-left: 10px;padding-right: 10px;">${data.name}</h3>
                    <p class="red font14" style="padding-left: 10px;">￥${data.price}</p>
                    <img src="${data.url1}" width="125" height="138" alt="" style="margin:10px auto">
                </a>
            </c:forEach>
        </div>
    </div>
</div>

<!--热门商品-->
<div class="width1200 center_yh hidden_yh">
    <div class="width100" style="height: 45px;line-height: 45px;border-bottom: 2px solid #0ab50a; margin-top: 20px;">
        <font class="left_yh font20">热门商品</font>
    </div>
    <div class="width100 hidden_yh" style="height: 480px;">
        <div class="normalPic">
            <c:forEach items="${rxs}" var="data" varStatus="l">
                <a href="${ctx}/item/view?id=${data.id}">
                    <h3 class="yihang c_33 font14 font100" style="padding-left: 10px;padding-right: 10px;">${data.name}</h3>
                    <p class="red font14" style="padding-left: 10px;">￥${data.price}</p>
                    <img src="${data.url1}" width="125" height="138" alt="" style="margin:10px auto">
                </a>
            </c:forEach>
        </div>
    </div>
</div>
    <%@include file="/common/ufooter.jsp"%>
</body>
</html>



















